...
public class GameLogicController extends Thread {
    ...
    public void run() {
                ...
                if (currentPlayer == noOfPlayers - 1) {
                    if (!gController.getPlayerDecision().equals("fold") && !gController.getPlayerDecision().contains("allin")) {
                        if (!(checkLivePlayers() > 1)) {
                            ...
                            for (int i = 0; i < 5; i++) {
                                gController.showCardsEndOfRound(i);
                            }
                            winnerDeclared = true;
                            break;
                        }
        ...
        // And show all players cards. /Andreas
  ...
  /**
   * A function which holds the game until user has clicked the OK-button in the winner dialog box.
   */
  private void hold(){
      while (hold) {
        try{
  ...
  /**
   * Method which sets the boolean variable hold to false.
   * Which will cause the hold-functions while-loop to break and let the game continue.
